...
export async function POST(request: NextRequest) {
  try {
    console.log('=== check-consent-status API 시작 ===')
    ...
    console.log('JWT 생성 완료:', { jwtLength: jwt.length, jwtStart: jwt.substring(0, 50) + '...' })
    
    // 서버에서 외부 SSDM API 호출
    const ssdmUrl = `${process.env.NEXT_PUBLIC_SSDM_URL}/api/check-consent-status`
    console.log('외부 SSDM API 호출:', { ssdmUrl, jwtLength: jwt.length })
    ...
    console.log('SSDM API 응답 헤더:', Object.fromEntries(response.headers.entries()))
